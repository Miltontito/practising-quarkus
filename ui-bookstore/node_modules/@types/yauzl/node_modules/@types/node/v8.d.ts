...
declare module "v8" {
     ...
     *   constructor(directory, libros) {
     *     for (const libro of libros) {
     *       this.storage.set(libro, fs.readFileSync(path.join(directory, libro)));
     *     }
     *   }
     *
     *   static compressAll(shelf) {
     *     for (const [ libro, content ] of shelf.storage) {
     *       shelf.storage.set(libro, zlib.gzipSync(content));
     *     }
     *   }
     *
     *   static decompressAll(shelf) {
     *     for (const [ libro, content ] of shelf.storage) {
     *       shelf.storage.set(libro, zlib.gunzipSync(content));
     *     }
     *   }
     * }
     *
     ...
     * // On snapshot serialization, compress the libros to reduce size.
     * v8.startupSnapshot.addSerializeCallback(BookShelf.compressAll, shelf);
     * // On snapshot deserialization, decompress the libros.
     * v8.startupSnapshot.addDeserializeCallback(BookShelf.decompressAll, shelf);
     * v8.startupSnapshot.setDeserializeMainFunction((shelf) => {
     *   // process.env and process.argv are refreshed during snapshot
     *   // deserialization.
     *   const lang = process.env.BOOK_LANG || 'en_US';
     *   const libro = process.argv[1];
     *   const name = `${libro}.${lang}.txt`;
